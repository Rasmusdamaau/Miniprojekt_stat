...
x_i <- 1:20
y_data <- c(-0.8, 1.15, 1.82, 2.89, 1.43, 0.59, 1.09, 3.00, 2.81, 3.02, 2.39, 3.50, 3.06, 2.41, 2.81, 3.95, 4.19, 5.94, 7.06, 4.55)
...
abline <- coef(lm(y ~ x, data = data.frame(x = 1:length(y_data), y =y_data)))
abline_plot <- geom_abline(intercept = abline[1], slope = abline[2])
test_plot_m1 <- data.frame(x = 1:length(y_data),y = seq(min(y_data, para_m1), max(y_data, para_m1), length = length(y_data)), y_data = y_data, fit_mean = para_m1)
ggplot(test_plot_m1, aes(x, y)) +
  geom_point(inherit.aes = FALSE, aes(x, y_data), color = "red", size = 3) +
  geom_point(inherit.aes = FALSE, aes(x, para_m1), color = "blue", size = 3) +
  labs(x = "index", y = "value") +
  abline_plot

test_plot_m2 <- data.frame(x = 1:length(y_data),y = seq(min(y_data, para_m2), max(y_data, para_m2), length = length(y_data)), y_data = y_data, fit_mean = para_m2)
ggplot(test_plot_m2, aes(x, y)) +
  geom_point(inherit.aes = FALSE, aes(x, y_data), color = "red", size = 3) +
  geom_point(inherit.aes = FALSE, aes(x, para_m2), color = "blue", size = 3) +
  labs(x = "index", y = "value") +
  abline_plot

test_plot_m2_optim <- data.frame(x = 1:length(y_data),y = seq(min(y_data, para_m2_optim), max(y_data, para_m2_optim), length = length(y_data)), y_data = y_data, fit_mean = para_m2_optim)
ggplot(test_plot_m2_optim, aes(x, y)) +
  geom_point(inherit.aes = FALSE, aes(x, y_data), color = "red", size = 3) +
  geom_point(inherit.aes = FALSE, aes(x, para_m2_optim), color = "blue", size = 3) +
  labs(x = "index", y = "value") +
  abline_plot

test_plot_m3 <- data.frame(x = 1:length(y_data),y = seq(min(y_data, para_m3), max(y_data, para_m3), length = length(y_data)), y_data = y_data, fit_mean = para_m3)
ggplot(test_plot_m3, aes(x, y)) +
  geom_point(inherit.aes = FALSE, aes(x, y_data), color = "red", size = 3) +
  geom_point(inherit.aes = FALSE, aes(x, para_m3), color = "blue", size = 3) +
  labs(x = "index", y = "value") +
  abline_plot

test_plot_m3_optim <- data.frame(x = 1:length(y_data),y = seq(min(y_data, para_m3_optim), max(y_data, para_m3_optim), length = length(y_data)), y_data = y_data, fit_mean = para_m3_optim)
ggplot(test_plot_m3_optim, aes(x, y)) +
  geom_point(inherit.aes = FALSE, aes(x, y_data), color = "red", size = 3) +
  geom_point(inherit.aes = FALSE, aes(x, para_m3_optim), color = "blue", size = 3) +
  labs(x = "index", y = "value")+
  abline_plot
